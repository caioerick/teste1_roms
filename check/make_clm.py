...
# griddata do matplotlib foi descontinuada
# assim, vou importar griddata do scipy.interpolate
...
print('==> Reading ASCII metadata file...\n')
run = run_setup('../para_testar.setup')
...
datafile = sp.loadmat(run.clm_filename)
...
grdfile  = netCDF4.Dataset('../input/' + run.run_name + '_grd.nc')

# assigning some variables from grid file
# lembrar de testar removendo todos os 'variables'
...
# qual interpolação nós vamos fazer aqui?
...
print('==> Interpolating temp from Z --> S coord...\n')
...
print('==> Interpolating salt from Z --> S coord...\n')
...
print('==> Interpolating U-Vel from Z --> S coord ...\n')
...
print('==> writting NetCDF climatology file...\n')

ncfile = netCDF4.Dataset('../input/' + run.run_name + filenamestr, mode='w',
...
print('==> ############################################\n')
print('==>  CLIM CONDITIONS FILE SUCCESSFULLY CREATED\n')
print('==> ############################################\n')
...
ncfile = netCDF4.Dataset('../input/' + run.run_name + filenamestr, mode='r')
...
map = Basemap(projection='merc',
            llcrnrlat=run.latmin,
            urcrnrlat=run.latmax,
            llcrnrlon=run.lonmin,
            urcrnrlon=run.lonmax,
            lat_ts=0,
            resolution='l')
...
for k in (0, 1 ,2):
    plt.subplot(2,3,k+1)
    map.pcolormesh(lon, lat, np.squeeze(temp[k,:,:]), vmin=0, vmax=30)
    map.drawcoastlines(zorder=5)
    map.drawcountries(zorder=4)
    map.fillcontinents(color=('#000000'), lake_color='#000000', zorder=3)
    map.drawparallels(np.arange(run.latmin, run.latmax, 5),
    labels=[0, 0, 0, 0], dashes=[1,3], zorder=6)
    map.drawmeridians(np.arange(run.lonmin, run.lonmax, 5),
    labels=[0, 0, 0, 0], dashes=[1,3], zorder=7)
    plt.title('Temp - lev = ' + str(lev[k])); plt.colorbar()

for k in (3, 4 ,5):
    plt.subplot(2,3,k+1)
    map.pcolormesh(lon, lat, np.squeeze(salt[k-3,:,:]), vmin=33, vmax=38)
    map.drawcoastlines(zorder=5)
    map.drawcountries(zorder=4)
    map.fillcontinents(color=('#000000'), lake_color='#000000', zorder=3)
    map.drawparallels(np.arange(run.latmin, run.latmax, 5), labels=[0, 0, 0, 0], dashes=[1,3], zorder=6)
    map.drawmeridians(np.arange(run.lonmin, run.lonmax, 5), labels=[0, 0, 0, 0], dashes=[1,3], zorder=7)
